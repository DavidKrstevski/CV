...
# Initially the models architecture was here.
...
y_pred = model.predict(X_test_array, verbose=1).flatten()
...
#%% 6a. Scatter Plot: Predictions vs Ground Truth
plt.figure(figsize=(7, 7))
sns.set_theme(style="whitegrid")
plt.scatter(y_test_array, y_pred, s=20, alpha=0.5, color="#EC407A", edgecolor="white", linewidth=0.7)
lims = [0, max(y_test_array.max(), y_pred.max()) + 5]
...
plt.title("Predicted vs. True Age (Test Set)", fontsize=14, weight="bold", color="#333333")
plt.legend()
plt.tight_layout()
plt.show()

#%% 6b. Residual Plot (Error vs True Age)
residuals = y_pred - y_test_array
plt.figure(figsize=(8, 5))
sns.scatterplot(x=y_test_array, y=residuals, color="#F48FB1", alpha=0.6, edgecolor="white")
plt.axhline(0, color="gray", linestyle="--")
plt.xlabel("True Age", fontsize=12)
plt.ylabel("Prediction Error (Pred - True)", fontsize=12)
plt.title("Residuals vs. True Age", fontsize=14, weight="bold", color="#333333")
plt.grid(True, linestyle="--", alpha=0.3)
plt.tight_layout()
plt.show()

#%% 6c. Error Distribution Histogram
plt.figure(figsize=(8, 5))
sns.histplot(residuals, bins=30, kde=True, color="#F06292", alpha=0.8)
plt.title("Distribution of Prediction Errors", fontsize=14, weight="bold", color="#333333")
...
#%% 6d. Mean Absolute Error per Age Bin (fixed)
AGE_BINS = [0, 5, 12, 18, 30, 45, 60, 80, 200]
bin_idx = np.digitize(y_test_array, AGE_BINS, right=False) - 1
bin_mae = []
bin_labels = []
...
for i in range(len(AGE_BINS) - 1):
    ...
    mask = bin_idx == i
    if np.any(mask):
        bin_mae.append(np.mean(np.abs(y_pred[mask] - y_test_array[mask])))
        label = f"{lo}–{hi-1}" if i < len(AGE_BINS) - 2 else f"{lo}+"
        bin_labels.append(label)

# Rosa-inspired palette (consistent with earlier visuals)
rosa_palette = ["#F9C5D5", "#F48FB1", "#EC407A", "#AD1457"]

plt.figure(figsize=(10, 5))
sns.barplot(x=bin_labels, y=bin_mae, palette=rosa_palette, alpha=0.9, legend=False)
plt.title("Average Absolute Error per Age Bin", fontsize=14, weight="bold", color="#333333")
...
#%% 6e. Identify Hardest Samples (Largest Errors)
abs_errors = np.abs(residuals)
sorted_idx = np.argsort(-abs_errors)  # descending
top_n = 8
worst_indices = sorted_idx[:top_n]

print(f"\nTop {top_n} hardest samples (largest prediction errors):")
for i in range(top_n):
    idx = worst_indices[i]
    print(f"#{i+1}: True={y_test_array[idx]:.1f}, Pred={y_pred[idx]:.1f}, Error={abs_errors[idx]:.1f}")
...
for i, idx in enumerate(worst_indices):
    ...
    plt.title(f"T:{int(y_test[idx])} / P:{int(y_pred[idx])}\nΔ={abs_errors[idx]:.1f}",
              fontsize=10, color="#4A4A4A")
    plt.axis("off")
plt.suptitle("Hardest Predictions (Largest Absolute Errors)", fontsize=14, color="#333333", weight="bold")
plt.tight_layout()
plt.show()

#%% 6f. Optional: Save predictions for external analysis
results_df = pd.DataFrame({
    "true_age": y_test_array,
    "pred_age": y_pred,
    "error": residuals,
    "abs_error": abs_errors,
})
results_df.to_csv("age_predictions_testset.csv", index=False)
print("\nSaved test predictions to 'age_predictions_testset.csv'")

#%% 6g. Bias & Error Analysis by Demographics
print("\n=== Bias / Error Analysis by Gender and Race ===")

# Subset test metadata
df_test = df.iloc[idx_test].copy()
df_test["true_age"] = y_test_array
df_test["pred_age"] = y_pred
df_test["abs_error"] = np.abs(df_test["true_age"] - df_test["pred_age"])

# --- Error by Gender ---
gender_stats = df_test.groupby("gender").agg(
    MAE=("abs_error", "mean"),
...
plt.figure(figsize=(6, 5))
sns.barplot(data=gender_stats, x="gender", y="MAE", palette=["#F9C5D5", "#F48FB1"])
plt.title("Mean Absolute Error by Gender", fontsize=14, weight="bold", color="#333333")
plt.xlabel("Gender")
...
# --- Error by Race ---
race_palette = ["#F9C5D5", "#F7A1C4", "#F48FB1", "#F06292", "#EC407A"]
race_stats = df_test.groupby("race").agg(
    MAE=("abs_error", "mean"),
    RMSE=("abs_error", lambda x: np.sqrt(np.mean(x**2))),
    Count=("abs_error", "count")
).reset_index()
...
plt.figure(figsize=(7, 5))
sns.barplot(data=race_stats, x="race", y="MAE", palette=race_palette)
plt.title("Mean Absolute Error by Race", fontsize=14, weight="bold", color="#333333")
plt.xlabel("Race")
plt.ylabel("MAE (years)")
plt.grid(True, linestyle="--", alpha=0.3)
plt.tight_layout()
...
#%% 6h. Calibration Plot (Predicted vs. True Averages per Age Bin)
...
calib_stats = (
    df_test.groupby("age_bin", observed=False)
    .agg(
...
sns.scatterplot(
    ...
    size="count",
    hue="count",
    palette="RdPu",  # ✅ replaced "cherry" with valid palette
    legend=False,
    s=80,
)

lims = [0, max(df_test["true_age"].max(), df_test["pred_age"].max()) + 5]
plt.plot(lims, lims, "k--", alpha=0.7)
plt.xlim(lims)
plt.ylim(lims)
plt.xlabel("Mean True Age (bin)", fontsize=12)
plt.ylabel("Mean Predicted Age (bin)", fontsize=12)
plt.title(
    "Calibration Plot: Predicted vs True Mean per Age Bin",
    fontsize=14,
    weight="bold",
    color="#333333",
)
